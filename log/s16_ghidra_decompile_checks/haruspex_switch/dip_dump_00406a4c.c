...
void dip_dump(undefined4 param_1,undefined4 param_2,undefined4 param_3,undefined4 param_4)
...
[32m    reg_read((local_70 + 0x40) * 4,&local_68,param_3,param_4,puVar4);[0m
  ...
  reg_write(0x80,0x8104);
  puts("hash   port(0:6)   rsp_cnt  flag  timer    dip-address       ATRD");
  local_70 = 0;
  while( true ) {
    if (0x7ff < local_70) {
      return;
...
[32m    reg_read(0x88,&local_60);[0m
    printf("     0x%4x",local_60 & 0xffff);
    printf("  0x%2x",(int)local_60 >> 0x10 & 0xff);
    printf("  %3d",local_60 >> 0x18);
[32m    reg_read(0x84,&local_64);[0m
    ip_to_str(auStack_18,local_64);
    printf("     %s",auStack_18);
    printf("  0x%8x\n",local_5c);
    if ((local_68 & 0x4000) != 0) break;
    reg_write(0x80,0x8105);
    usleep(5000);
    local_70 = local_70 + 1;
  }
