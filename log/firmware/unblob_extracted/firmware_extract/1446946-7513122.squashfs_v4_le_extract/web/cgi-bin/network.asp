...
IPv4PPPoEPWD=decodeBase64("<% getIPv4Info(PPPoEPWD_FAKE); %>");
...
HTTPAuthenticate=decodeBase64("<% getHTTPInfo(Authenticate)%>");
RTSPPort=decodeBase64("<% getRTSPInfo(Port)%>");
RTSPAuthenticate=decodeBase64("<% getRTSPInfo(Authentication)%>");
...
function init(bReinit)
{
	...
	if (!bReinit)
	{
		document.getElementById("pNetworkChange").style.display = "none";
		document.getElementById("sNetworkChange").style.display = "none";
		loadText();
	}
	setConfig();
	modifyLanSettingUI();
...
function loadText()
{
	...
	setText(document.getElementById("sAuthenticationSetting"), item_name[I_AUTHENTICATION_SETTING]);
	setText(document.getElementById("sNetworkChange"), item_name[I_PLEASE_USE_NEW_IP_LOGIN]);
	...
	document.getElementById("selectRTSPAuthenticate").options[0].text = item_name[I_DISABLE];
}
function setConfig()
{
	...
	document.getElementById("selectHTTPAuthenticate").value = HTTPAuthenticate;
	document.getElementById("selectRTSPAuthenticate").value = RTSPAuthenticate;
	pwd_dirty=0;
}
...
function save_setting_callback(http_request)
{
	if (http_request.readyState == 4) {
        if (http_request.status == 200) {
            document.getElementById("pNetworkChange").style.display = "";
			document.getElementById("sNetworkChange").style.display = "";
        	resetDefaultConfig();
...
function save_setting()
{
	if (document.getElementsByName("radio_ipMode")[1].checked)
	{
  	...
  	if (document.getElementById("input_https_enable").checked)
  	{
  		var iHTTPPort = parseInt(document.getElementById("input_http_port").value, 10);
  		var iHTTPSPort = parseInt(document.getElementById("input_https_port").value, 10);
  		var iRTSPPort = parseInt(document.getElementById("input_rtsp_port").value, 10);
  		if (iHTTPPort == iHTTPSPort || iHTTPPort == iRTSPPort)
  		{
  			document.getElementById("input_http_port").select();
  			alert(pop_msg[PMSG_INVALID_PORT_NUMBER]);
  			return;
  		}
  		if (iHTTPSPort == iRTSPPort)
  		{
  			document.getElementById("input_https_port").select();
  			alert(pop_msg[PMSG_INVALID_PORT_NUMBER]);
  			return;
	...
	params += "&RTSPAuthenticate=" + encodeURIComponent(document.getElementById("selectRTSPAuthenticate").value);
...
function resetDefaultConfig()
{
	...
	HTTPAuthenticate = document.getElementById("selectHTTPAuthenticate").value;
	RTSPAuthenticate = document.getElementById("selectRTSPAuthenticate").value;
	//setConfig();
	//modifyLanSettingUI();
	//onPPPoEEnable();
	//onHTTPSEnable();
    			  ...
    			  <td>
    				<div id="maincontent">
        			  ...
        			  <div align=center>
        				...
        				<span>&nbsp;&nbsp;</span>
        				<INPUT id="btn_not_save_setting" type="button" width="80" value=" Don't Save Settings " onClick="document.getElementById('pNetworkChange').style.display = 'none';document.getElementById('sNetworkChange').style.display = 'none';init(true);">
        			  </div>
        			  <div align="left">
        			    <p id="pNetworkChange" style="display: none"></p>
        				<span id="sNetworkChange" style="display: none; color:red">Please use new ip to login.</span>
        			  </div>
      				</div>
      				<div class="box">
        				    ...
        				    <div id="div_pppoe_setting">
        				      <table id="id_box_pppoe" cellspacing="3" cellpadding="3" BORDER="0" class=maintext>
            				    ...
            				    <tr>
              					  ...
              					  <td><INPUT type="password" size=18 maxlength=32 id="input_PPPoEPWD" name="input_PPPoEPWD" onclick="this.value='';pwd_dirty=1;" onchange="pwd_dirty=1"></td>
            				    </tr>
            				  </table>
            				</div>
            			  </TD>	
      				...
      				<div class="box">
        			  <h2><span id="sAuthenticationSetting">Authentication settings</span></h2>
        			  <table cellpadding="2" cellspacing="1" border="0" width="100%" bgcolor="white" bordercolor="#FFFFFF">
        			    <TR>
          				  <TD width="163px"><span>HTTP</span></TD>
          				  <TD>
          				    <select id="selectHTTPAuthenticate">
        				...
        				<TR>
          				  <TD width="163px"><span>RTSP</span></TD>
          				  <TD>
          				    <select id="selectRTSPAuthenticate">
        			  ...
        			  <H4><span id="sBonjourSetting">BONJOUR SETTINGS</span></H4>
        				...
        				<span>&nbsp;&nbsp;</span>
        				<INPUT id="btn_not_save_setting2" type="button" width="80" value=" Don't Save Settings " onClick="document.getElementById('pNetworkChange').style.display = 'none';document.getElementById('sNetworkChange').style.display = 'none';init(true);">
      				  </p>
       				  <!-- === END MAINCONTENT === -->
    				</div>
    			  </td>
