...
function loadText()
{
	...
	document.getElementById("sTitleInfo").innerHTML = des_item_name[D_TITLE_INFO_FLASH];
...
function save_setting()
{
	...
	if (document.getElementById("input_motion_enable").checked == true)
	{
		...
		/*if (live_obj)
		{*/
			strMotionBlockInfo  = $('#video')[0].getVal();    
		//}
...
//flash player ===================================================================
      ...
      function swf_ready()
      {
		var nharr = location.host.split(":");
		var nowhost = nharr[0];

	$('#video')[0].video_url("http://" + nowhost + ":" + remote_port + "/cgi/mjpg/mjpeg.cgi");  
	$('#video')[0].video_play(); 

	$('#video')[0].setVal(MDBlockMBlock);
      }
