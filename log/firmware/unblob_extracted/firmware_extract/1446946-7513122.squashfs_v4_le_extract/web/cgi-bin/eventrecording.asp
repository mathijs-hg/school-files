...
document.write("<script language=\"JavaScript\" type=\"text/javascript\" src=\"lang/" + language + "/lang_eventrecording.js\"></scr" + "ipt>");
</script>
<script type="text/javascript" language="Javascript">
var g_showEffect=0;
...
function loadText()
{
	setText(document.getElementById("sTitle"), item_name[I_VIDEOCLIP_TITLE]);
	setText(document.getElementById("sVideoClipTitle"), item_name[I_VIDEOCLIP]);
	setText(document.getElementById("sVideoClip"), item_name[I_VIDEOCLIP_ITEM]);
	...
	setText(document.getElementById("sRecordingType"), item_name[I_RECORDING_TYPE]);
	setText(document.getElementById("sRecordingType2"), item_name[I_VIDEOCLIP_ITEM]);
	setText(document.getElementById("sFileFormat"), item_name[I_FILE_FORMAT]);
	setText(document.getElementById("sPreEventRecording"), item_name[I_PREEVENT_RECORDING]);
	setText(document.getElementById("sSeconds"), item_name[I_SECONDS]);
	setText(document.getElementById("sMaximumDuration"), item_name[I_MAXIMUM_DURATION]);
	setText(document.getElementById("sSeconds2"), item_name[I_SECONDS]);
	...
	for (var i = 0; i < document.getElementById("select_file_format").length; i++)
	{
		document.getElementById("select_file_format").options[i].text = option_content[O_AVI + i];
	}
}
function setConfig()
{
...
*/
	...
	{
		if (iffun("Profile") == "event_recording")
	...
	var max_recording_length = 	(parseInt(iffun("PostbufferTime"), 10) + parseInt(iffun("PrebufferTime"), 10)).toString();
	document.getElementById("select_rec_length").value = max_recording_length;
	document.getElementById("select_prebuffer_time").value = iffun("PrebufferTime");
	document.getElementById("select_encode_format").value = iffun("EncodeFormat");
	document.getElementById("select_file_format").value = iffun("FileFormat");
	...
	var findGUISection=0;
	for (var i = 0; i < lstarr.length; i++)
	{
		if (lstarr[i].name == "event_recording_gui")
		{
			findGUISection=1;
	...
	if (findGUISection == 0)
	{
		for (var i = 0; i < lstarr.length; i++)
		{
			if (lstarr[i].name == "event_recording")
			{
				if (lstarr[i].weekday == null)
					continue;
				document.getElementsByName("input_sche_weekday")[parseInt(lstarr[i].weekday, 10)].checked = true;
...
{
	var url = "/cgi/admin/eventrecording.cgi";
	var params = "";
	/*
	...
	*/
	if (document.getElementById("input_enable").checked == true)
	{
		...
		if (saveSchedule)
		{
			params += "&Profile=event_recording";
			var iWeekend = 0;
			for (var i = 0; i < document.getElementsByName("input_sche_weekday").length; i++)
			{
		...
		params += "&EncodeFormat=" + document.getElementById("select_encode_format").value;
		params += "&FileFormat=" + document.getElementById("select_file_format").value;
		var postBufferTimeInt = parseInt(document.getElementById("select_rec_length").value, 10) - parseInt(document.getElementById("select_prebuffer_time").value, 10);
		if (postBufferTimeInt < 0)
			postBufferTimeInt = 0;
		var postBufferTimeString = postBufferTimeInt.toString();	
		params += "&PostbufferTime=" + postBufferTimeString;
		params += "&PrebufferTime=" + document.getElementById("select_prebuffer_time").value;
	}
	else
	{
		params += "MotionEnableTrigger=0";
...
<body topmargin="0" leftmargin="0" rightmargin="0" bottommargin="0" bgcolor="#FFFFFF" style="display: none" onload="document.getElementById('if_data').src = 'eventrecording_data.asp?' + g_token">
    			...
    			<tr>
    			  <td>
    				<div id="maincontent">
      				  <!-- === BEGIN MAINCONTENT === -->
      				  <div id="box_header"> 
      				    <h1><span id="sTitle">Video Clip</span></h1>
      					<span id="sTitleInfo">Video Clip is a feature to send video clips via FTP or E-Mail when a trigger is activated.</span>    
        			    <div align=center>
        			      <span>&nbsp;</span>
        			      <p></p>
      				  ...
      				  <div class="box">
        			    <h2><span id="sVideoClipTitle">Video Clip</span></h2>
        			    <table cellpadding="2" cellspacing="1" border="0" width="534" bgcolor="white" bordercolor="#FFFFFF" style="display: ">
        			      <TR>
          				    <TD width="130px">&nbsp;<span id="sVideoClip">Video Clip</span></TD>
          				    <TD>
          				      <span>&nbsp;</span>
        				...
        				<div id="div_recording_setting">
        				  ...
        				  <table cellpadding="2" cellspacing="1" border="0" width="534" bgcolor="white" bordercolor="#FFFFFF">  
							<TR>
							  <TD width="130px"><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><span id="sTime">Time</span><span>:</span></TD>
							  <TD>
          				        <span>&nbsp;&nbsp;</span>
						  ...
						  <table cellpadding="2" cellspacing="1" border="0" width="534" bgcolor="white" bordercolor="#FFFFFF">  
							<TR style="display: none">
							  <TD width="130px"><span>&nbsp;&nbsp;&nbsp;&nbsp;</span>Recording Type</TD>
							  <TD>
          				        <span>&nbsp;&nbsp;&nbsp;</span>Source
								<span>&nbsp;</span>
								<select id="select_encode_format">
								  <option value="2">H.264</option>
								  <option value="0">MJPEG</option>
								</select>
							  </TD>
							</TR>
							<TR style="display: none">
          				      <TD width="130px"><span>&nbsp;&nbsp;&nbsp;&nbsp;</span><span id="sRecordingType">Video Clip Type</span></TD>
          				      <TD style="display: ">
								<span>&nbsp;&nbsp;</span>
								<span id="sFileFormat">File Format</span>
								<span>&nbsp;</span>
								<select id="select_file_format">
								  <option value="1">Avi, .avi</option>
								  <option value="2">Mp4, .mp4</option>
								</select>
							  </TD>
							</TR>
							<TR>
							  <TD width="130px"><span>&nbsp;&nbsp;&nbsp;&nbsp;</span><span id="sRecordingType2">Video Clip</span></TD>
							  <TD>
								<span>&nbsp;&nbsp;</span>
								<span id="sPreEventRecording">Pre-event recording</span>
								<span>:</span>
								<span>&nbsp;</span>
								<select id="select_prebuffer_time">
							...
							<TR>
							  ...
							  <TD>
								<span>&nbsp;&nbsp;</span>
								<span id="sMaximumDuration">Maximum duration</span>
								<span>:</span>
								<span>&nbsp;</span>
								<select id="select_rec_length">
								  <option value="5">5</option>
								...
								<span id="sSeconds2">Seconds</span>
							  </TD>
							</TR>
							<TR>
        ...
        <table cellpadding="2" cellspacing="0" border="0" bgcolor="#FFFFFF" bordercolor="#FFFFFF" bordercolordark="#FFFFFF" bordercolorlight="#FFFFFF">
    	  <tr>
      		<td id="help_text">
			    ...
			    <b>Trigger by</b><br>
			    <b>MotionBegin</b><br>
			    video clipping after a motion is detected.<br>
			    <b>SoundBegin</b><br>
			    video clipping after sound is detected.<br><br>
			    <b>Video Clip Type</b><br>
			    ...
			    <b>Target</b><br>
			    You can select the target as FTP or E-mail for the video clip.
			  </span>
      		</td>
    	  </tr>
    	</table>
