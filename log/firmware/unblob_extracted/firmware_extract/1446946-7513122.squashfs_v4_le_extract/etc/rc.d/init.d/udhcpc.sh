...
run_dhcp_process()
{
	...
	while [ $dhcp_count -lt $dhcp_retry ]; do
		...
		ifconfig $iface up
                if [ "$ipAddr" == "" ];then
                        ...
                        /sbin/udhcpc -n -r "$ipAddr" -i $iface -x hostname:"$hostname" -p $pidfile
                fi
	...
	if [ $retval = 0 ]; then
		/usr/bin/iwpriv wlan0-vxd set_mib shortGI20M=1
		/usr/bin/iwpriv wlan0-vxd set_mib shortGI40M=1
			...
			run_dhcp_process
				...
				if [ $IndicatedLED = 0x01 ]; then
					...
					temp1=`msger hwmon 0 pwrled 0 1 0 5`
                        ...
                        elif [ $dhcp_ret == 1 ] && [ $phylink = "wired" ]; then
                                `iwpriv wlan0 set_mib func_off=1`
                                `/etc/rc.d/init.d/udhcpd.sh stop`
                        fi
