...
link_up_all_ports () {
	local phy_port_1=$(uci -c /etc/profile.d/ get profile.switch.phy_port_1)
	local phy_port_2=$(uci -c /etc/profile.d/ get profile.switch.phy_port_2)

	for port in $phy_port_1 $phy_port_2
	do
		set_phy_power $port on
	done
