...
proto_6to4_setup() {
	...
	network_echo $LOG_PRI_NOTICE "6to4" "$iface start dial"
	( proto_add_host_dependency "$cfg" 0.0.0.0 )
	local wanif
	if ! network_find_wan wanif; then
		proto_notify_error "$cfg" "NO_WAN_LINK"
		proto_setup_failed "$cfg"
		network_echo $LOG_PRI_ERR "6to4" "no wan link,exit"
		return
	fi
	...
	[ -z "$ipaddr" ] && {
		if ! network_get_ipaddr ipaddr "$wanif"; then
			...
			proto_setup_failed "$cfg"
			network_echo $LOG_PRI_ERR "6to4" "no wan address,exit"
			return
		fi
	}
...
proto_6to4_teardown() {
	network_echo $LOG_PRI_NOTICE "6to4" "teardown"
	local cfg="$1"
	local link="6to4-$cfg"
