...
nat_load_rule_vs() {
	...
	for element in $addr; do
		...
		if [ $index -eq 1 ]; then
			...
			internal_ip=$internal_ip"."$element
		fi		
	done
	if [ -z "$rule_vs_internal_ip" ]; then
		rule_vs_ipaddr=$internal_ip"."$rule_vs_ip
	else
		rule_vs_ipaddr=$rule_vs_internal_ip
	fi
...
nat_rule_vs_operation() {
	...
	# flush all flow, or connection by port forwarding won't stop.
