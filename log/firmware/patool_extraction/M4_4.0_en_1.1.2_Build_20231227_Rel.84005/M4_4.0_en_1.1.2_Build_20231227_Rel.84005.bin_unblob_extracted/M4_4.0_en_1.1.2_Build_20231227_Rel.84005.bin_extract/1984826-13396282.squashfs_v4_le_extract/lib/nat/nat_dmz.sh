    ...
    nat__do_dmz() {
	   ...
	   [ -n "$proto" ] && [ "$proto" == "pptp" ] && {
                [ -n "$wan_sec_ip" -a "$wan_sec_ip" != "0.0.0.0" -a "$wan_sec_ip" != "-" ] && {  
                    ...
                    fw add 4 n ${n_dmz} ACCEPT  ^ \
                         { -d ${wan_sec_ip} -p gre } #gre packet should be unpacked once before DNAT
                }  
            }
	    [ -n "$proto" ] && [ "$proto" == "l2tp" ] && {
                [ -n "$wan_sec_ip" -a "$wan_sec_ip" != "0.0.0.0" -a "$wan_sec_ip" != "-" ] && {  
                    fw add 4 n ${n_dmz} ACCEPT ^ \
                         { -d ${wan_sec_ip} -p udp -m udp --sport 1701 } 
                }  
            }
		    ...
		    local proto=$(uci_get_state nat env ${i}_proto)
                    ...
                    [ -n "$ifname" -a -n "$dev" ] && {
                        [ -n "$wan_ip" -a "$wan_ip" != "0.0.0.0" ] && {                            
                            nat__do_dmz $ifname $nat_dmz_ipaddr $wan_ip $zone $proto $wan_sec_ip                             
                        }      
