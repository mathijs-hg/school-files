...
pppv6_generic_init_config() {
	...
	proto_config_add_string "keeplive"
...
pppv6_generic_setup() {
	...
	json_get_vars ipv4 keeplive username password pppd_options ip_mode ip_config dns_mode ifname
	...
	local interval="${keeplive##*[, ]}"
	[ "$interval" != "$keeplive" ] || interval=5
...
#	[ "$dns_mode" == "static" ] && dnsarg=""
	...
	proto_run_command "$config" /usr/sbin/pppd \
		...
		ipparam "$config" \
		${keeplive:+lcp-echo-interval $interval lcp-echo-failure ${keeplive%%[, ]*}} \
		nodefaultroute noaccomp nopcomp ipv6 \
		${dnsarg:+"$dnsarg"} \
