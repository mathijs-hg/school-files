...
fw_configure_interface() {
	...
	[ "$action" == "add" ] && {
		...
		#if iface is wan,zone-wan firewall should exist no matter if it is up,by yaopeiyong
		if [ "$isMobile" = "yes" ]; then
			network_echo $LOG_PRI_NOTICE "firewall" "-> adding $iface status=$status"
			[ "$iface" == "wan" ] || [ "$iface" == "wanb" ] || [ "$status" == 1 ] || [ -n "$aliasnet" ] || return 0
		else
			[ "$iface" == "wan" ] || [ "$status" == 1 ] || [ -n "$aliasnet" ] || return 0
		fi
	}
	...
	fw__do_rules() {
		...
		case "$mode/$subnet" in
			...
			*/*:*) network_echo $LOG_PRI_NOTICE "firewall" "zone $zone does not support IPv6 address family, skipping"; return ;;

			# Need v4 while zone is v6
			*/*.*) network_echo $LOG_PRI_NOTICE "firewall" "zone $zone does not support IPv4 address family, skipping"; return ;;
	...
	[ -n "$old_zones" ] && {
		...
		for z in $old_zones; do
			...
			for n in ${old_subnets:-""}; do
				network_echo $LOG_PRI_NOTICE "firewall" "removing $iface ($old_ifname${n:+ alias $n}) from zone $z"
	...
	load_zone() {
		...
		network_echo $LOG_PRI_NOTICE "firewall" "adding $iface ($ifname${aliasnet:+ alias $aliasnet}) to zone $zone_name"
