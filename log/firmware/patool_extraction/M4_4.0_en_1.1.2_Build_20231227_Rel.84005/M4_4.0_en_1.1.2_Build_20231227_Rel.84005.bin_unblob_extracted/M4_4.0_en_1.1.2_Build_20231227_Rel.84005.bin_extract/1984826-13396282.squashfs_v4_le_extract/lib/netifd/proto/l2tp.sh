...
proto_l2tp_setup() {
	...
	network_echo $LOG_PRI_NOTICE "l2tp" "$iface start dial"
	local server
	...
	[ -n "$serv_addr" ] || {
		...
		network_echo $LOG_PRI_ERR "l2tp" "could not resolve server ip,exit."
...
#	xl2tpd-control connect l2tp-${config}
	...
	network_echo $LOG_PRI_DEBUG "l2tp" "start xl2tpd,${username:+user \"$(escaped_str "$username")\" password \"$(escaped_str "$password")\"} ifname \"l2tp-$config\""

	# make sure pppd killed before setup (Bug 73649) 
	ps | grep -v grep | grep pppd | grep l2tp | sed 's/  */ /g' | grep -o '[^ ].*' | cut -d " " -f1 | xargs kill -9
	proto_run_command "$config" nice -n -10 /usr/sbin/xl2tpd -D \
		-C $controlfile \
		-c $conffile
...
proto_l2tp_teardown() {
	...
	network_echo $LOG_PRI_NOTICE "l2tp" "$interface teardown"
	...
	case "$ERROR" in
		...
		;;
		11|19)
			network_echo $LOG_PRI_ERR "l2tp" "auth fail"
			proto_notify_error "$interface" AUTH_FAILED
			proto_block_restart "$interface"
		;;
		2)
			network_echo $LOG_PRI_ERR "l2tp" "invalid options"
