...
#	fi
	...
	network_echo $LOG_PRI_DEBUG "ppp" "start pppd,${username:+user "$username"} ${password:+password "$password"} ${mru:+mtu $mru mru $mru}"
	proto_run_command "$config" /usr/sbin/pppd \
		nodetach ifname "${proto:-ppp}-$config" \
		ipparam "$config" \
		${keepalive:+lcp-echo-interval $interval lcp-echo-failure ${keepalive%%[, ]*}} \
		${ipv6:++ipv6} \
		nodefaultroute noaccomp nopcomp \
		$dnsarg \
		${ipaddr:+"$ipaddr:"} \
		$demand maxfail 1 \
...
ppp_generic_teardown() {	
	local interface="$1"
	network_echo $LOG_PRI_NOTICE "ppp" "teardown $interface "
	case "$ERROR" in
		11|19)
			proto_notify_error "$interface" AUTH_FAILED
			json_get_var authfail authfail
			network_echo $LOG_PRI_ERR "ppp" "auth fail"
			if [ "${authfail:-0}" -gt 0 ]; then
				proto_block_restart "$interface"
			fi
		;;
		2)
			network_echo $LOG_PRI_ERR "ppp" "invalid options"
...
proto_pppoe_setup() {	
	local config="$1"
	local iface="$2"
	network_echo $LOG_PRI_NOTICE "pppoe" "$iface start dial"
...
proto_pppoe_teardown() {
	network_echo $LOG_PRI_NOTICE "pppoe" "teardown" "$@"
	ppp_generic_teardown "$@"
}
...
proto_pptp_setup() {
	...
	local iface="$2"
	local parent=""
	network_echo $LOG_PRI_NOTICE "pptp" "$iface start dial"
	...
	[ -n "$serv_addr" ] || {
		...
		network_echo $LOG_PRI_ERR "pptp" "Could not resolve server address"
		err_code=`cat /tmp/resolveip_error`
		if [ "$err_code" == "2" ]; then
            echo "102" > /tmp/xxtpConnectionError
            sleep 5
...
proto_pptp_teardown() {
	network_echo $LOG_PRI_NOTICE "pptp" "teardown" "$@"
	ppp_generic_teardown "$@"
	#Control PPTP dialing hardware acceleration
