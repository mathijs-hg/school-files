...
nat_dns_param_to_rule_tcp()
{
    ...
    if [ -z "$param" ]; then
        ...
        if [ -z "$lan_addr" ] || [ "$lan_addr" = "0.0.0.0" ] || [ -z "$dns_server" ] || [ "$dns_server" = "0.0.0.0" ]
        then
            dnsproxy_rule=""
        else
            dnsproxy_rule="-d ${lan_addr}/32 -p tcp -m tcp --dport 53 --to-destination $dns_server"
        fi
    fi
