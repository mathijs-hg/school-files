...
proto_dslite_setup() {
    ...
    local aftr_pri1="gw.transix.jp"
    local aftr_pri2="dslite.v6connect.net"
    local aftr_pub1="dgw.xpass.jp"
    local can_ping=0

    local mtu ttl peeraddr ip6addr dns_server dns_pri dns_snd server mode
    ...
    config_get mode $cfg AFTR_type

    echo "server: $server" > /dev/console
    echo "mode: $mode" > /dev/console
    ...
    #[ -z "$server" ] && [ "auto" == "$mode" ] && {
        ### Below actions will restart dhcpv6 and resend DHCPv6 option aftr to get AFTR name form DHCPv6 ###
        ### But Japanese carriers doesn't support this feature now.
    ...
    if [[ "auto" == "$mode" ]]; then
        if [[ -z "$server" ]]; then
            server="$aftr_pri1"
        elif [[ "$aftr_pri1" == $server && -z "$peeraddr" ]]; then
            server="$aftr_pri2"
        elif [[ "$aftr_pri2" == $server && -z "$peeraddr" ]]; then
            server="$aftr_pub1"
        elif [[ "$aftr_pub1" == $server && -z "$peeraddr" ]]; then
            server="$aftr_pri1"
        fi
        uci set network."$cfg".AFTR_name="$server"
        uci set network_sync."$cfg".AFTR_name="$server"
        uci commit network
    ...
    [ -n "$server" ] && [ -z "$peeraddr" ] && {
	    ...
	    [ -n "$peeraddr" ] && {
            ...
            if [[ "auto" == "$mode" ]]; then
                if [[ "$aftr_pub1" == $server && 1 != $can_ping ]]; then
                    echo "Can't ping to $server now, need restart dslite.sh" > /dev/console
                    proto_notify_error "$cfg" "MISSING_ADDRESS"
                    sleep 5
                    proto_setup_failed "$cfg"
                    return
                fi
                echo "------------------in auto mode, now need to save aftr name--------------" > /dev/console
                saveconfig a
            fi
		}
...
proto_dslite_init_config() {         
    ...
    proto_config_add_string "AFTR_type"
