...
noip_update()
{
    local prog="$1"
    local url="$2"
    local current_ip_for_update="$3"
    local is_publicip="$4"
    
    local myip="&myip="
    config_get wan_bind "noip" wan_bind
    # If we can't get public ip, we can only update with no ip like when wan_bind=disable
    # unquote this to use external ip
    #[ "$wan_bind" != "disable" ] && [ "$is_publicip" = "true" ] && {
    [ "$wan_bind" != "disable" ] && {
        url="$url""$myip""$current_ip_for_update"
    }
    
    #echo "noip update final url is $url" >/dev/console
    noip_result=1
    
    if [ -n "$prog" ] && [ -n "$url" ]
    then
        if ${prog} -v ${url} 2>$NOIP_HTTP_ERR_PATH; then
            noip_result=1
        else
            noip_result=0        
        fi
...
noip_parse_retcode()
{
    local service_id="$1"
    local http_res
    http_res=$(cat $NOIP_HTTP_ERR_PATH | grep -v "User-Agent: curl/" | grep -o "curl")

    noip_code=''
    if [ -n "$http_res" ] && [ "$http_res" == "curl" ]; then
        # Only check 4xx http error code
        local http_code
        http_code=$(grep -o "4[0-9][0-9]" < $NOIP_HTTP_ERR_PATH)
        case $http_code in
            ...
            ;;
            *) 
                #echo "noip https code is $http_code" >/dev/console
        ...
        local result
        result=$(cat /var/run/dynamic_dns/${service_id}.retcode|awk -F ' ' '{print $1}')
        case $result in
            good)       noip_code=$NOIP_RETCODE_GOOD ;;
            nochg)      
			local ip
            ip=$(cat /var/run/dynamic_dns/${service_id}.retcode|awk -F ' ' '{print $2}')
			if [ -n "$ip" ]; then
...
{
    local s
    s=$(uci_get_state ddns noip state)
    
    config_get domain "noip" domain 
    [ -n "$s" ] && {
	#do not check state so we can check if update is successful when return_code is 8002
        #if [ $s -eq $NOIP_STATE_CONNECTING ]; then
            network_get_dnsserver dns "$1"
            for d in $dns; do
                [ -n "$d" ] && {
                    local ns
                    ns=$(dnslookup -t $NOIP_NSLOOKUP_TMO $domain $d|head -n 1|grep -o "$ip_regex")
                    [ "$ns" == "$2" ] && {
