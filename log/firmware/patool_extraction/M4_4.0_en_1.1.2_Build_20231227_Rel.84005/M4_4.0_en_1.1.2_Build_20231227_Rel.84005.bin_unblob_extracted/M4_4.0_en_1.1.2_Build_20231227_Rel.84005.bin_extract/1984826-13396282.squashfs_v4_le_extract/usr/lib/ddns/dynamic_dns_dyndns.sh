...
dyndns_update()
{
    ...
    if [ -n "$prog" ] && [ -n "$url" ]
    then
        # add '-A tplink' to add user-agent name
        if ${prog} -v -0 -H "host: members.dyndns.org" -H "Accept:" "${url} " 2>$DYNDNS_HTTP_ERR_PATH; then
            dyndns_result=1
        else
            dyndns_result=0        
        fi
...
dyndns_parse_retcode()
{
    local service_id="$1"
    local http_res
    http_res=$(cat $DYNDNS_HTTP_ERR_PATH | grep -v "User-Agent: curl/" | grep -o "curl")

    dyndns_code=''
    if [ -n "$http_res" ] && [ "$http_res" == "curl" ]; then
        # Only check 4xx http error code
        local http_code
        http_code=$(grep -o "4[0-9][0-9]" < $DYNDNS_HTTP_ERR_PATH)
        case $http_code in
            ...
            ;;
            *) 
                #echo "dyndns https code is $http_code" >/dev/console
...
dyndns_set_state()
{
    ...
    if [ $code -eq $DYNDNS_RETCODE_GOOD ] || [ $code -eq $DYNDNS_RETCODE_NOCHG ]; then
    ...
    elif [ $code -ge $DYNDNS_RETCODE_NOHOST ] && [ $code -le $DYNDNS_RETCODE_NOT_YOURS ]; then
...
{
    local s
    s=$(uci_get_state ddns dyndns state)
    ...
    [ -n "$s" ] && {
        #do not check state so we can check if update is successful when return_code is 8002
            ...
            for d in $dns; do
                [ -n "$d" ] && {
                    local ns
                    ns=$(dnslookup -t $DYNDNS_NSLOOKUP_TMO $domain $d|head -n 1|grep -o "$ip_regex")
                    [ "$ns" == "$2" ] && {
